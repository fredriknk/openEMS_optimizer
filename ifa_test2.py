...
Sim_Path = os.path.abspath('tmp_IFA')
Sim_CSX = 'IFA.xml'

# Show structure
showCad = True #set true to show cad
...
gndplane_position=-0.36#referenced from top layer
substrate_cells = 6

# IFA parameters
ifa_h = 5.586       # height of short circuit stub
ifa_l = 19.588      # length of radiating element
...
stop = start + np.array([ifa_wf, ifa_h, 0])
ifa.AddBox(start=start, stop=stop, priority=10)

mesh.AddLine('y', np.linspace(start[1], stop[1], substrate_cells+1))

via_size=0.3

# Short circuit stub
start = np.array([-ifa_fp, 0-via_size, 0]) + tl
stop = start + np.array([-ifa_w1, ifa_h+via_size, 0])
ifa.AddBox(start=start, stop=stop, priority=10)

# Via
start = np.array([-ifa_fp-ifa_w1/2, 0-via_size/2, 0]) + tl
stop = start + np.array([0, 0, gndplane_position])
#ifa.AddCyl(start=start, stop=stop, priority=10)
gnd.AddCylinder(start=start, stop=stop,radius=via_size/2, priority=10)
mesh.AddLine('y', np.linspace(start[1]-via_size, stop[1]+via_size, 5))
mesh.AddLine('x', np.linspace(start[0]-via_size, stop[0]+via_size, 5))
mesh.AddLine('z', np.linspace(start[2]-via_size, stop[2]+via_size, 5))
# Radiating element
...
FDTD.AddEdges2Grid(dirs='xy', properties=ifa, metal_edge_res=0.2)

# Apply the excitation & resistor as a current source
start = np.array([0, 0, 0]) + tl
stop = start + np.array([ifa_wf, 0, gndplane_position])
...
port = FDTD.AddLumpedPort(1, feed_R, start, stop, 'z', 1.0, priority=5, edges2grid='xz')
# Finalize the mesh
